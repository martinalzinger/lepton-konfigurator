...
.dtitle .sub{font-family:var(--mono);font-size:12px;color:var(--red);letter-spacing:.04em;margin-top:9px;font-weight:500}
...
<section class="hero noprint">
  <div class="hero-in">
    <div class="kick">Alzinger Recyclingtechnik · Lepton 5100</div>
    <h1 id="heroTitle">Angebot erstellen</h1>
    <div class="hsub">Mobile Sternsiebanlage LEPTON 5100 · Serie 2025</div>
...
(function(){
 ...
 function renderAngebot(r){return docHead("Angebot-Nr.")+addrBlock()+'<div class="dtitle">ANGEBOT<span class="u"></span><span class="sub">Mobile Sternsiebanlage LEPTON 5100 \u00b7 Serie 2025</span></div>'+salutation()+machineSpec("",true)+linesHTML(r.sel,"Ausgew\u00e4hlte Optionen",true)+totalsHTML(r,"Gesamtkaufpreis (netto) inkl. aller gew\u00e4hlten Ausstattungsvarianten")+termsHTML()+legalBlock(r.tax)+signSingle();}
 function renderGebraucht(r){var anr=v("k_anrede"),nach=v("k_nach"),s=(anr==="Herr"&&nach)?"Sehr geehrter Herr "+esc(nach)+",":((anr==="Frau"&&nach)?"Sehr geehrte Frau "+esc(nach)+",":"Sehr geehrte Damen und Herren,");return docHead("Angebot-Nr.")+addrBlock()+'<div class="dtitle">ANGEBOT \u2013 GEBRAUCHTMASCHINE<span class="u"></span><span class="sub">Mobile Sternsiebanlage LEPTON 5100</span></div><div class="salut"><p>'+s+'</p><p>gerne unterbreiten wir Ihnen \u2013 freibleibend \u2013 das nachfolgende Angebot \u00fcber die unten beschriebene <b>Gebrauchtmaschine</b>.</p></div>'+gdataBlock()+machineSpec("Gebrauchtmaschine",false)+linesHTML(r.sel,"Ausstattung",false)+totalsGebraucht(r)+termsHTML()+legalBlock(r.tax)+signSingle();}
 function renderKaufvertrag(r){
  ...
  return docHead("Vertrag-Nr.")+'<div class="dtitle">KAUFVERTRAG<span class="u"></span></div>'+vp+'<div class="sec-h">Gegenstand des Vertrages</div><p class="ptxt">Der Verk\u00e4ufer verkauft und der K\u00e4ufer kauft zu den nachfolgenden Bedingungen folgende konfigurierte Neumaschine:</p>'+machineSpec("",true)+linesHTML(r.sel,"Ausstattung",true)+totalsHTML(r,"Kaufpreis (netto)")+termsHTML()+legalBlock(r.tax)+signTwo();
 }
