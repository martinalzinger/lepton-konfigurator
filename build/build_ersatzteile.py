...
(function(){
 ...
 function buildDoc(){
  ...
  L.forEach(function(l,i){var line=l.p.price>0?l.p.price*l.q:0;if(l.p.price>0){sub+=line;anyP=true;}
   rows+='<tr><td>'+(i+1)+'</td><td class="zoll">'+esc(l.p.art)+'</td><td class="nm">'+esc(pName(l.p))+'</td><td class="zoll">'+esc(l.p.zoll||"—")+'</td><td class="r">'+l.q+'</td><td class="r">'+(l.p.weight>0?esc(fmtKg(l.p.weight*l.q)):"—")+'</td><td class="r">'+(l.p.price>0?money(l.p.price):t("price_request"))+'</td><td class="r">'+(l.p.price>0?money(line):"—")+'</td></tr>';});
  var tw=totalWeight();
